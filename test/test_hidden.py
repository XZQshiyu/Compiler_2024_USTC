...
def eval():
    ...
    for level in suite:
        ...
        for case in tqdm(cases, desc=f"Processing {level_name}"):
            ...
            try:
                result = subprocess.run([EXE_PATH, "-o", TEST_PATH + ".ll", "-emit-llvm", "-mem2reg", "-const-prop", "-loop-inv-hoist", "-loop-analysis", TEST_PATH + ".sy"], stderr=subprocess.PIPE, timeout=45)
