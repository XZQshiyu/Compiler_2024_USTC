...
for case in $testcases; do
	...
	if [ $debug_mode = true ]; then
		bash -c "sysy -mem2reg -const-prop -emit-llvm $case -o $ll_file" >>$LOG 2>&1
	fi
	# sysy compile to .s
	bash -c "sysy -S -mem2reg -const-prop $case -o $asm_file" >>$LOG 2>&1
	check_return_value $? 0 "CE" "sysy compiler error" || continue

	# gcc compile asm to executable
	riscv64-linux-gnu-gcc -lc -static \
